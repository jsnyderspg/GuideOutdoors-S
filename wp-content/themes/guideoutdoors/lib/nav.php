...
class Roots_Nav_Walker extends Walker_Nav_Menu {
  ...
  function getRecentPosts($cat, $post_type = "") // Getting recent posts from tips and adventures
    {
	  global $post;
// added 'post' below for News/Blogs drop-down
    $args = array(
			'post_type' => array( 'tips', 'adventures', 'post' ),
			'orderby' => 'post_date',
			'posts_per_page' => 4,
			'post_status' => 'publish',
			'orderby' => 'post_date',
    ...
    $recentTitle = "Recent Tips & Adventures";
    if($post_type && $post_type != ''){
      $args['post_type'] = $post_type;
      $post_type_obj = get_post_type_object( $post_type );
      $recentTitle = "Recent " . $post_type_obj->labels->name;
    }
	  $loop = new WP_Query($args);
	   $tips = '';

	   $tips .= '<span class="tips_posts featured-title">' . $recentTitle . '</span>';
	  ...
	  $tips .='</td>';
	   ...
	   if(($post_type_obj->labels->name)=='Recipes'){
		$tips .= '<div class="button_hide" style="float:left;margin-top:65px;text-align:centre;margin-left: -32px;"><a class="btn buttonSecondary"  style="width:141px;font-size:13px!important;height:37px" href="/share/recipe/">SUBMIT YOUR OWN</a></div>';
	   }
	   if(($post_type_obj->labels->name)=='Trophies'){
		$tips .= '<div class="button_hide" style="float:left;margin-top:65px;text-align:centre;margin-left: -32px;"><a class="btn buttonSecondary"  style="width:141px;font-size:13px!important;height:37px" href="/share/trophy/">SUBMIT YOUR OWN</a></div>';
	   }
	   if(($post_type_obj->labels->name)=='Views of The Day'){
		$tips .= '<div class="button_hide" style="float:left;margin-top:65px;text-align:centre;margin-left: -32px;"><a class="btn buttonSecondary"  style="width:141px;font-size:13px!important;height:37px" href="/share/view/">SUBMIT YOUR OWN</a></div>';
	   }
	   
	   if(($post_type_obj->labels->name)=='Recipes'){
			$tips .= '<div style="float:right;"><a href="/recipes/" style="margin-right:6px" class="view-all-cat">View All</a></div>';
	   }
	   else if(($post_type_obj->labels->name)=='Trophies'){
			$tips .= '<div style="float:right;"><a href="/trophies/" style="margin-right:6px" class="view-all-cat">View All</a></div>';
	   }
	   else if(($post_type_obj->labels->name)=='Views of The Day'){
			$tips .= '<div style="float:right;"><a href="/view_of_day/" style="margin-right:6px" class="view-all-cat">View All</a></div>';
	   }
	   else{
			$tips .= '<div style="float:right;"><a href="'.get_category_link( $cat ).'" style="margin-right:6px" class="view-all-cat">View All</a></div>';
       }
		wp_reset_query();
  ...
  function getUserPosts($type, $limit,$str) // Getting user info w.r.t to post/posty types.
  {
	  ...
	  {
		  $result .= '<span class="expert-heading">Guide Outdoors Featured Experts</span>';
	  }
	  else
	  {
...
//        $mega_class = ($is_post_type_archive) ? " mega-left" : "";
        ...
        <div class="featured-outer js-test">'.'</div>

...
class Roots_Nav_Walker extends Walker_Nav_Menu {
  ...
  function getFeaturedPost($cat,$post_type = "") // Getting featured post from parent category
  {
    ...
    $args = array(
			...
			'orderby' => 'post_date',
			'order' => 'DESC',
      'post_type' => array('post','recipes','tips','adventures','gear','buyers_guides')
    );
    if($cat && $cat != ''){
      $args['cat'] = $cat;
    }
    if($post_type && $post_type != ''){
	  ...
	  $loop = new WP_Query($args);
		...
		$featured .= '<h1 class="featured-title">Featured</h1>';
  ...
  function getRecentPosts($cat, $post_type = "") // Getting recent posts from tips and adventures
  {
    ...
    if($post_type && $post_type != ''){
	  ...
	  $loop = new WP_Query($args);
	   ...
	   $tips .= '<h1 class="tips_posts featured-title">Recent Tips & Adventures</h1>';
  ...
  function getUserPosts($type, $limit) // Getting user info w.r.t to post/posty types.
  {
	  global $wpdb;
	  $result = '';
	  if($type[0] == 'tips' || $type[0] == 'adventures')
	  {
		  $result .= '<h1 class="expert-heading">Guide Outdoors Experts</h1>';
	  }
	  else
	  {
		  $result .= '<h1 class="expert-heading">Bloggers</h1>';
	  }
    $type = implode("' OR post_type = '", $type);
	  $query = "SELECT post_author, COUNT(*) as post_count, display_name, u.ID
                FROM wp_posts p JOIN wp_users u ON (u.ID = p.post_author)
                WHERE (post_type = '$type') AND (post_status = 'publish' OR post_status = 'private')
                GROUP BY post_author ORDER BY post_count DESC LIMIT 0,$limit";
	  $query_res = mysql_query($query);
		  ...
		  while($rec = mysql_fetch_array($query_res))
		  {
			  ...
			  $result .= '<li><table class="expert-table"><tr><td>'.get_avatar($rec['post_author'], 38).'</td><td class="link-expert">'.$author_url.'<br />'.$rec['post_count'].' Posts</td></tr></table></li>';
			  if($count == 6)
			  {
    ...
    if ($item->is_dropdown && ($depth === 0)) {
      ...
      { //if item is experts
        if($item->title == 'Experts')
        {
            ...
            <div class="outdoor-experts-outer">'.$this->getUserPosts(array('tips','adventures'), 12).'</div>
            <div class="blogger-experts-outer">'.$this->getUserPosts(array('post'), 6).'</div>', $item_html);
        }

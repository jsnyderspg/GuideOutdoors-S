<?php
/*
add_action( 'admin_init', 'spg_mass_field_update' );

function spg_mass_field_update(){
    if($_GET['mass'] == 'true'){
        echo "Updating ...";
        $post_types = spg_get_featured_types();
        $args = array(
        ...
        foreach($items->posts as $item){
            ...
            if($meta['featured_general'][0] !== '1'){
                update_post_meta( $item->ID, 'featured_general', '0' );
            }
        }
    }
}
...
function spg_add_custom_types( $query ) {
    if($query->is_main_query() && empty( $query->query_vars['suppress_filters'] )) {
        ...
        if ($query->is_search or $query->is_feed) {
                /* turned this off because it was causing the backend to search sportsman's guide when searching gear reviews 
                if($query->query_vars['post_type'] == "gear") {
                    if($query->is_search){
                        $s = $query->query_vars['s'];
                        header("Location: http://www.sportsmansguide.com/net/search/eSearch.aspx?SearchTerms=$s");
                        exit;
                    }else{
                ...
                // Everything
                /*elseif($query->query_vars->post_type == "all") { */
				if($query->query_vars->post_type == "all") {
...
//  SHORTCODES
// ************************************************
add_shortcode( 'carousel', 'add_shortcode_carousel' );
function add_shortcode_carousel($atts){
    $args = array(
...
function slider(){
    ...
    <ul class="homeSlider">
        ...
        <?php 
            $img_desktop = wp_get_attachment_image_src( get_post_thumbnail_id( $slide->ID ), 'slide-desktop' );
            $img_mobile  = wp_get_attachment_image_src( get_post_thumbnail_id( $slide->ID ), 'slide-mobile' );
        ?>
        <li>
        	<a href='<?php echo get_permalink( $slide->ID ); ?>'>
        		<span class="img" data-src-desktop="<?php echo $img_desktop[0]; ?>" data-src-mobile="<?php echo $img_mobile[0]; ?>">
                    <noscript><?php echo get_the_post_thumbnail( $slide->ID, 'slide-desktop' ); ?></noscript>
                </span>
...
function carousel($post_type = array('post'), $limit = 6, $cat = '', $geo = ''){
    ...
    <?php if(count($slides->posts) >=1): ?>
    	<hr />
        <h2 class="sectionTitle"><?php echo ($post_type == 'post') ? "From the Blog" : $slides->post_type_obj->labels->name; ?></h2>
        ...
        <div class="post-carousel" data-page="2" data-type="<?php echo $post_type; ?>" data-limit="<?php echo $limit; ?>" data-geo="<?php echo $geo; ?>" data-cat="<?php echo $cat; ?>">
            ...
            <?php foreach($slides->posts as $slide): ?>
                <li>
                    ...
                    <a href='<?php echo get_permalink( $slide->ID ); ?>' class="readMore">READ MORE &rsaquo;</a>
                </li>
            <?php endforeach; ?>
            </ul>
...
function get_carousel($post_type = array('post'), $limit = 6, $cat = '', $geo = ''){
    ...
    $carousel_query = "
        ...
        "FROM wp_posts  ";
    if($cat || $geo){
        $carousel_query .= "INNER JOIN wp_term_relationships ON (wp_posts.ID = wp_term_relationships.object_id) ";
    }
    $carousel_query .= "
         INNER JOIN wp_postmeta ON (wp_posts.ID = wp_postmeta.post_id) ".
        "WHERE 1=1  ";
    if($geo || $cat){
        //$carousel_query .= "AND ( wp_term_relationships.term_taxonomy_id IN (".get_queried_object()->term_id.") ) ";
        $carousel_query .= "AND ( wp_term_relationships.term_taxonomy_id IN (".get_queried_object()->term_taxonomy_id.") ) ";
    }
    $carousel_query .= "
             AND wp_posts.post_type = '". $post_type ."' ".
            "AND (wp_posts.post_status = 'publish' OR wp_posts.post_status = 'private') ".
            "AND (wp_postmeta.meta_key = 'featured_general' ) ".
        "GROUP BY wp_posts.ID ".
        "ORDER BY wp_postmeta.meta_value+0 DESC,wp_posts.post_date DESC ".
        "LIMIT 0, " . $limit;
    if(!$cat && !$geo){
        $viewAllHref = $post_type == "post" ? get_permalink( get_option( 'page_for_posts' ) ) : get_post_type_archive_link( $post_type );
    }else{
    ...
    $carousel->posts = $wpdb->get_results($carousel_query, OBJECT);
    //$temp = new WP_Query($args);
    //$carousel->posts = $temp->posts;
    //pre($carousel_query);
    //pre($temp);
    return $carousel;
}
...
*/
    ...
    <?php if(count($slides)>=1): ?>
	    <hr />
          <?php
			$cat_name = get_queried_object()->slug;
				//echo "<pre>";print_r($sector); die;
				...
				if(!empty($post)){
				?>
					<h2 class="sectionTitle">Trophies</h2>
					<a class="btn buttonPrimary" href="<?php echo site_url();?>/geo/<?php echo $cat_name; ?>/?type=trophies">View All</a>
					<div class="clearfix"></div>
					<div class="post-carousel"> 
					<ul class="customSlider">
							<?php
							foreach ( $post as $get_page ) 
							{
							
							?>
							<li>
								<a href='<?php echo site_url(); ?>/trophies/<?php echo $get_page->post_name;?>'>
									<div class="thumb-container">
										<div class="thumb-title">
											<h3><?php echo $get_page->post_title; ?></h3>
										</div>
										<?php 
										if(has_post_thumbnail($get_page->ID)) {
											  echo get_the_post_thumbnail($get_page->ID,'thumbnail');
											}
										else {
											echo '<img src="' . get_bloginfo( 'stylesheet_directory' ) . '/TSG_placeholder_image.jpg" />';
										}
										?>
									</div>
								</a>
								<p class="entry-summary">
									<span class='description'><?php echo (strlen($get_page ->post_content) > 110) ? substr($get_page -> post_content, 0, 107) .  '...' : $get_page -> post_content; ?></span><br />
									<a href='<?php echo site_url(); ?>/trophies/<?php echo $get_page->post_name;?>'  class="readMore">READ MORE</a>
								</p>
							</li>
						<?php } ?>
					</ul>
			...
			<?php 
			       //$get_pages = $wpdb->get_results("SELECT * FROM `wp_posts` WHERE  (`post_type` = 'view_of_day') and (`post_status` ='publish') and (`posts_per_page`=>'5')");
					/*		
						$query = new WP_Query( array(
							'post_type' => 'view_of_day',
							'posts_per_page'=>5,
							'post_status' =>'publish',
							));
						...
						//echo "<pre>"; print_r($posts); 
							...
							foreach ( $posts as $get_pages ) 
							{
							     //echo $get_pages ->post_content; die;
							*/
							?>
				...
				<?php 
				$cat_name_view_of = get_queried_object()->slug;
				$query = new WP_Query(array(
						'post_type' => 'view_of_day',
						'paged' => $paged,
						'posts_per_page' => 5,
				...
				if(!empty($posts)){
				?>
				<h2 class="sectionTitle">View of the day</h2>
				<a class="btn buttonPrimary" href="<?php echo site_url();?>/geo/<?php echo $cat_name_view_of; ?>/?type=view_of_day">View All</a>
				<div class="clearfix"></div>
				<div class="post-carousel">   
				<ul class="customSlider">
					<?php
					...
					?>
					<li>
						<a href='<?php echo site_url(); ?>/view_of_day/<?php echo $get_pages->post_name;?>'>
							<div class="thumb-container">
								<div class="thumb-title">
									<h3><?php echo $get_pages->post_title; ?></h3>
								</div>
								<?php //echo $get_pages['guid'];
									 if (has_post_thumbnail($get_pages->ID)) {
											 echo get_the_post_thumbnail($get_pages->ID,'thumbnail');
										}
									 else {
											echo '<img src="' . get_bloginfo( 'stylesheet_directory' ) . '/TSG_placeholder_image.jpg" />';
										}
						...
						<p class="entry-summary">
							<span class='description'><?php echo (strlen($get_pages ->post_content) > 110) ? substr($get_pages -> post_content, 0, 107) .  '...' : $get_pages -> post_content; ?></span><br />
							<a href='<?php echo site_url(); ?>/view_of_day/<?php echo $get_pages->post_name;?>'  class="readMore">READ MORE</a>
						</p>
					</li>
					<?php 
						}
            ...
            <?php foreach($slides as $user):?>
                ...
                <li>
                    ...
                    <p class="entry-summary">
                    	...
                    	<a href='<?php echo $link; ?>'  class="readMore">READ MORE &rsaquo;</a>
                    </p>
                </li>
            <?php endforeach; ?>
            </ul>
...
function state_select(){
    ...
    ?>
    <?php if(count($state_select) > 0): ?>
        <h3 style="font-size:13px;">News, Adventures & Links By State</h3>
        <form class="state-form" action="../">
        	<select class="state-select form-control" name="stateSelect">
	            <option value="#">Filter Guide Outdoors by State</option>
...
function spiffs($featured = false){
    ...
    if($spiffs[0]):
    ?>
    <div class="spiffs-container <?php echo ($featured) ? 'row' : ''; ?>">
    ...
    <?php setup_postdata($spiff); ?>
        ...
        $spiffClasses[] = 'spiff';
        if($featured){
            $spiffClasses[] = 'featuredSpiff';
        }
        if($thumb){
            $spiffClasses[] = 'has-thumb';
        }
        $spiffClasses = implode(" ", $spiffClasses);
        ?>
        <div class="<?php echo $spiffClasses; ?>">
            <?php if($featured): ?>
                <div class="thumb-container">
                    <div class="thumb-title">
                        <?php if($showTitle): ?>
                    ...
                    <?php echo make_link($thumb, $link); ?>
                </div>
            <?php else: ?>
            <?php if($showTitle): ?>
            <div class="spiff-title">
            	<h3><?php echo make_link($spiff -> post_title, $link); ?></h3>
            </div>
            <?php endif; ?>
...
function author_card(){
 ...
 ?>
     ...
     <?php /* $desc = strip_tags($meta['description'][0]); */ ?>
		<?php $desc = get_the_author_meta('description', $uID); ?>
        ...
        <div class="author-card">
            ...
            <div class="author-bio">
                ...
                <a href="#" class="readMore">Read More &rsaquo;</a>
            	<?php endif; ?>
            	<?php if(is_single()): ?>
...
function view_of_the_day(){
    ...
    <div class="view-of-the-day widget">
        ...
        <?php foreach($views->posts as $view): ?>
            <?php $meta = get_post_meta($view->ID); ?>
    	<a href='<?php echo get_permalink($view -> ID); ?>'>
    		<?php echo get_the_post_thumbnail($view -> ID, 'thumbnail-large'); ?>
    	</a>
    	<a href='<?php echo get_permalink($view -> ID); ?>'>
    		<h4><?php echo $meta['view_location'][0]; ?></h4>
    	</a>
        <em>Submitted by <?php echo $meta['photographer_name'][0]; ?></em>
        <?php endforeach; ?>
        <div>
        	<a class="btn buttonPrimary" href="/view_of_day/">View All</a><a class="btn buttonSecondary" href="/share/view/">Submit Your Own</a>
        </div>
    </div>
    <div class="clearfix"></div>
    <?php endif; ?>
...
function featured_trophy(){
    ...
    <div class="featured-trophy widget">
        ...
        <?php endforeach; ?>
        <div>
        	<a class="btn buttonPrimary" href="/trophies/">View All</a><a class="btn buttonSecondary" href="/share/trophy/">Submit Your Own</a>
        </div>
    </div>
    <div class="clearfix"></div>
    <?php endif; ?>
...
// -----------------------------------------------
//  custom_scripts()
//  SOME OF THESE ARE TEMPORARY SCRIPTS/STYLES FOR DEVELOPMENT PURPOSES ONLY
// -----------------------------------------------

function custom_scripts() {
    wp_register_script('jquery_mobile_script', get_template_directory_uri() . '/assets/js/device.min.js', array(), '0fc6af96786d8f267c8686338a34cd37', true);
    wp_enqueue_script('jquery_mobile_script');
...
/*  ------------------------------------------------------
    Script and Functions Added by Adeel
    ------------------------------------------------------
*/  

//Update Post Views Each Time Post has been viewed
function setPostViews($postID) {
    $count_key = 'spg_view_count';
